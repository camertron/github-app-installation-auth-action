...
var import_lru_cache = __nccwpck_require__(9802);
function getCache() {
...
async function main() {
    ...
    const auth = (0,dist_node.createAppAuth)({ appId, privateKey, clientId, clientSecret });
    const installationAuth = await auth({
        type: 'installation',
        installationId: installationId
    });
    const token = installationAuth.token;
    external_node_fs_namespaceObject.writeFileSync(process.env["GITHUB_OUTPUT"], `access-token=${token}`);
}
